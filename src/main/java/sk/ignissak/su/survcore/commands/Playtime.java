...
public class Playtime implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0) {
            ...
            sender.sendMessage("§fTvoj playtime je §a" + hodky + "h§f a §a" + minutky + "m§f. §7(#" + sql.getTopPlaytime(p.getName()) + ")");
            return true;
        }
        else {
            ...
            if (sql.getPlaytimeData(args[0]) == 0) {
                sender.sendMessage("§c§l(!) §cTento hráč sa zrejme nikdy nepripojil alebo je neplatný");
                return true;
            }
            ...
            sender.sendMessage("§fPlaytime hráča §2" + args[0] + "§f je §a" + hodkyy + "h §fa§a " + minutkyy + "m§f. §7(#" + sql.getTopPlaytime(args[0]) + ")");
            return true;
        }
